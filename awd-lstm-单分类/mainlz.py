...
wri=SummaryWriter(log_dir='D:/EEG/tensorboard/temp/qjt_50-250_8——de')
...
parser.add_argument('--data', type=str, default='qjt_VCEEGall_0-250ms_all_torch_8.mat',
...
parser.add_argument('--batch_size', type=int, default=128, metavar='N',
                    help='batch size')
parser.add_argument('--bptt', type=int, default=251,
...
parser.add_argument('--when', nargs="+", type=int, default=[150,300],
...
# Load data
###############################################################################'''
def getdatafrommat(rootpath=args.rootpath,one_fold=args.one_fold,batch_size=args.batch_size): 
    dat_num                = 20224   # n*sample*channel
    for elem in os.listdir(rootpath):
        if '{}'.format(args.data) in elem:
            ...
            if one_fold:
                valx_2     = xdata[:,int(len(xdata[1])//128*128*0.9)//batch_size*batch_size:dat_num,:]
                train_xdata= xdata[:,0:int(len(xdata[1])//128*128*0.9)//batch_size*batch_size,:]
                valy_2     = ydata[int(len(ydata)//128*128*0.9)//batch_size*batch_size:dat_num]
                train_ydata=ydata[0:int(len(ydata)//128*128*0.9)//batch_size*batch_size]
                return train_xdata,train_ydata,valx,valy,valx_2,valy_2,xdata,ydata
...
###############################################################################'''
'''主要测试函数  不使用最终模型看看''' 
def evaluate(data_source,data_source_target, batch_size=2104):
...
def evaluate_all_data(data_source,data_source_target, batch_size=128):
        ...
        while e < len(data_source_target):
            data, targets                          = get_batchlz(data_source,data_source_target, e ,batch_size, evaluation=True)
            output, hidden, rnn_hs, dropped_rnn_hs = model(data,None,return_h=True)
            pred_y                                 = torch.max(output, 1)[1].data
            accuracy                               = (pred_y == targets).float().sum() / len(targets)
            e                                     +=batch_size
...
###############################################################################'''
if args.one_fold:
    train_data,train_label,val_data,val_label,val_data2,val_label2,all_xdata,all_ydata=getdatafrommat()
    ...
    val_data2,val_label2        = torch.from_numpy(val_data2),torch.from_numpy(val_label2.reshape(len(val_label2))).long()
    if args.cuda:
        ...
        val_data2,val_label2    = val_data2.cuda(),val_label2.cuda()
else:
    train_data,train_label,val_data,val_label  = getdatafrommat()
    val_data,val_label                         = torch.from_numpy(val_data),torch.from_numpy(val_label.reshape(len(val_label))).long()
    train_data,train_label                     = torch.from_numpy(train_data),torch.from_numpy(train_label.reshape(len(train_label))).long()
    if args.cuda:
        train_data,train_label                 = train_data.cuda(),train_label.cuda()#和nn.Module不同，调用tensor.cuda()只是返回这个tensor对象在GPU内存上的拷贝，而不会对自身进行改变。因此必须对tensor进行重新赋值，即tensor=tensor.cuda().
...
#        optimizer.param_groups[0]['lr'] = lr2 * seq_len / args.bptt
        ...
        data, targets                          = get_batchlz(train_data,train_label, i, args.batch_size)
...
#        wri.add_scalar('loss',loss,epoch*179+batch)
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            print('ACC={:3.2f} | epoch {:3d} | {:5d}/{:5d} batches | lr {:05.5f} | ms/batch {:5.2f} | '
                    '平均loss {:5.2f} | ppl {:5.2f} | bpc {:5.3f}'.format(
                accuracy,epoch, batch, lzarg, optimizer.param_groups[0]['lr'],
...
#                optimizer.param_groups[0]['lr']=lrr[epoch-1]
            ...
            if args.optimizer == 'adam':
                if val_loss < stored_loss and epoch > 50:
                    model_save('{}_ADAM_loss={:.2f}_acc={:.2f}_epoch={}_model'.format(args.save,val_loss,val_acc,epoch))
                    model_save('{}_ADAM_loss={:.2f}_acc={:.2f}_epoch={}_para'.format(args.save,val_loss,val_acc,epoch),0,1)
                ...
                if epoch in args.when:
                    print('Saving model before learning rate decreased')
                    model_save('{}_ADAMLr={}_acc={:.2f}_loss={:.2f}_epoch={}_model'.format(args.save, optimizer.param_groups[0]['lr'],val_acc,val_loss,epoch))
                    model_save('{}_ADAMLr={}_acc={:.2f}_loss={:.2f}_epoch={}_para'.format(args.save, optimizer.param_groups[0]['lr'],val_acc,val_loss,epoch),0,1)
